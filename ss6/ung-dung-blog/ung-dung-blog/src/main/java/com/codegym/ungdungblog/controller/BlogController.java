...
public class BlogController {
    ...
    public ModelAndView search(@PageableDefault(value = 5) Pageable pageable,
        ...
        Page<Blog> blogs;
        if (categorySearch.get().equals("All Categories")) {
            blogs = blogService.findAllByNameContaining(searchString, pageable);
        } else {
            blogs = blogService.findAllByNameContainingAndCategory(searchString, categorySearch.get(), pageable);
        }
        mv.addObject("blogs", blogs);
        mv.addObject("categories", categories);
        mv.addObject("search", searchString);
